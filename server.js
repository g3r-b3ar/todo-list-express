...
app.get('/', async (request, response) => {
	...
	const itemsLeft = await db
		.collection('todos')
		.countDocuments({ completed: false }) // counts how many to do items are not completed yet
	response
		.render('index.ejs', { items: todoItems, left: itemsLeft }) // renders the items into html using ejs
		// db.collection('todos').find().toArray()      // if not for line above would require the following code
		// .then(data => {
...
app.post('/addTodo', (request, response) => {
	...
	db.collection('todos')
		.insertOne({ thing: request.body.todoItem, completed: false }) // parses to do item from request and inserts into the database
		.then(result => {
...
app.put('/markComplete', (request, response) => {
	...
	db.collection('todos')
		.updateOne(
			...
			{
				// updates the database object with todo set to true
				$set: {
